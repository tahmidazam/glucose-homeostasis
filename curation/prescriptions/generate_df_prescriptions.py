...
def generate_df_prescriptions(engine: sqlalchemy.Engine) -> pd.DataFrame:
    cache_filename = "df_classified_prescriptions"
    ...
    df_prescriptions = pd.merge(
        left=df_prescriptions, right=df_ndc_lookup, how="left", on="ndc"
    )

    # Counting unidentified prescriptions after NDC lookup.
    df_unidentified_prescriptions = df_prescriptions[
        df_prescriptions[ColumnKey.RXCUI.value].isna()
    ]
    ...
    df_prescriptions = pd.merge(
        left=df_prescriptions, right=df_gsn_lookup, how="left", on=["gsn"]
    )
    df_prescriptions[ColumnKey.RXCUI.value] = df_prescriptions["rxcui_x"].fillna(
        df_prescriptions["rxcui_y"]
    )
    ...
    df_name_lookup = generate_name_lookup(
        df_unidentified_prescriptions[ColumnKey.DRUG.value], column_key=ColumnKey.DRUG
    )
    df_prescriptions = pd.merge(
        left=df_prescriptions, right=df_name_lookup, how="left", on=["drug"]
    )
    df_prescriptions[ColumnKey.RXCUI.value] = df_prescriptions["rxcui_x"].fillna(
        df_prescriptions["rxcui_y"]
    )
    ...
    unidentified_prescription_count += (
        ("drug name", len(df_unidentified_prescriptions)),
    )
    ...
    unidentified_prescription_count += (
        ("generic drug name", len(df_unidentified_prescriptions)),
    )
    ...
    df_prescriptions = pd.merge(
        left=df_prescriptions,
        right=df_class_lookup,
        how="left",
        on=ColumnKey.RXCUI.value,
    )
    ...
    df_prescriptions.to_feather(path=path)
