...
def generate_df_demographics(
    ...
    :return:
    """
    cache_filename = "df_demographics"
    ...
    df_demographics: pd.DataFrame = pd.merge(
        left=pd.merge(
            left=df_icu_stays,
            right=df_admissions,
            on=[ColumnKey.HOSPITAL_ADMISSION_ID.value, ColumnKey.SUBJECT_ID.value],
            how="inner",
        ),
        right=df_patients,
    ...
    df_demographics = pd.merge(
        ...
        right=df_glucose_insulin[
            [ColumnKey.ICU_STAY_ID.value, ColumnKey.FIRST_ICU_STAY.value]
        ],
    ...
    demographics_len_history += (
        ("merge:glucose insulin dataset", len(df_demographics)),
    )
    ...
    df_demographics: pd.DataFrame = df_demographics[
        (
            df_demographics[ColumnKey.LENGTH_OF_STAY.value]
            > Filter.LENGTH_OF_STAY_LOWER_BOUND.value
        )
        & (
            df_demographics[ColumnKey.LENGTH_OF_STAY.value]
            < Filter.LENGTH_OF_STAY_UPPER_BOUND.value
        )
    ]
    ...
    df_demographics = df_demographics[
        df_demographics[ColumnKey.HOSPITAL_ADMISSION_ID.value].isin(
    ...
    df_heights_weights[ColumnKey.HEIGHT.value] = (
        df_heights_weights.groupby(ColumnKey.ICU_STAY_ID.value)[ColumnKey.HEIGHT.value]
        .ffill()
        .bfill()
    )

    df_heights_weights[ColumnKey.WEIGHT.value] = (
        df_heights_weights.groupby(ColumnKey.ICU_STAY_ID.value)[ColumnKey.WEIGHT.value]
        .ffill()
        .bfill()
    )
    ...
    df_heights_weights = (
        df_heights_weights.groupby(ColumnKey.ICU_STAY_ID.value).last().reset_index()
    )

    df_demographics = pd.merge(
        left=df_demographics,
        right=df_heights_weights,
    ...
    df_demographics.dropna(
        subset=[ColumnKey.HEIGHT.value, ColumnKey.WEIGHT.value], inplace=True
    )
    ...
    df_demographics = df_demographics[
        df_demographics[ColumnKey.HEIGHT.value] < Filter.HEIGHT_UPPER_BOUND.value
    ]
    df_demographics = df_demographics[
        df_demographics[ColumnKey.WEIGHT.value] < Filter.WEIGHT_UPPER_BOUND.value
    ]
    ...
    plot_count_history(
        count_history=demographics_len_history,
        title="Demographics record count",
        upper_x_lim=16000,
        left=0.4,
    )

    df_demographics.to_feather(path)
